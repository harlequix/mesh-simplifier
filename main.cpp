...
int main(int argc, char *argv[]){
	...
	Graphicobject object=reader.readFile("teapot.obj");
	 /*for (Edge* i : object.edge_list) {
		object.cost_queue.push(PriorityItem(i, i->cost(i->vert1)));
		object.cost_queue.push(PriorityItem(i, i->cost(i->vert2)));
	}*/
	...
	foo.cost_queue.push(PriorityItem(&e1, e1.length()));
	foo.cost_queue.push(PriorityItem(&e2, e2.length()));
	foo.cost_queue.push(PriorityItem(&e3, e3.length()));
	foo.cost_queue.push(PriorityItem(&e4, e4.length()));
	foo.cost_queue.push(PriorityItem(&e5, e5.length()));
	foo.cost_queue.push(PriorityItem(&e6, e6.length()));
	foo.cost_queue.push(PriorityItem(&e7, e7.length()));
	foo.cost_queue.push(PriorityItem(&e8, e8.length()));
	foo.cost_queue.push(PriorityItem(&e9, e9.length()));
	foo.cost_queue.push(PriorityItem(&e10, e10.length()));
	foo.cost_queue.push(PriorityItem(&e11, e11.length()));
	foo.cost_queue.push(PriorityItem(&e12, e12.length()));
	foo.cost_queue.push(PriorityItem(&e13, e13.length()));
	foo.cost_queue.push(PriorityItem(&e14, e14.length()));
	foo.cost_queue.push(PriorityItem(&e15, e15.length()));
	...
	foo.show();
	//foo.tri_counter = foo.triangle_list.size();
	//std::cout << foo.triangle_list.size();
	//object.simplifytil(1);
	...
	std::cout << os2.str();
	foo.show();
	std::cout << foo.tri_counter;*/
	//Edge* foobar = object.cost_queue.top().content;
	//object.simplify(foobar->vert1, foobar->vert2);
	//object.cost_queue.pop();
	//object.cost_queue.pop();
	//foobar = object.cost_queue.top().content;
	//object.simplify(foobar->vert1, foobar->vert2);
	////object.show();
	/*Vertex v1(-1,0,0);
	...
	std::cout << foo.triangle_list.size();*/
	/*while(!object.cost_queue.empty()){
		PriorityItem i = object.cost_queue.top();
		object.cost_queue.pop();
